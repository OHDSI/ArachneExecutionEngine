...
@Slf4j
@Service
public class CdmMetadataService {
    ...
    private static final String REGEX_V5 = "^V(5+|6+)_.*";

    @Autowired
    private SqlMetadataServiceFactory sqlMetadataServiceFactory;
    @Autowired
    private VersionDetectionServiceFactory versionDetectionServiceFactory;
    ...
    public void extractMetadata(DataSourceUnsecuredDTO dataSource, File dir) throws SQLException, IOException {
        try {
            ...
            try {
                ...
                log.error("Failed to detect CDM Version, {}", e.getMessage());
            }
            log.debug("[{}] CDM version: {}", dataSource.getType(), firstNonNull(cdmVersion, "not detected"));
            ...
            log.warn("Could not extract CDM metadata", e);
        }
    }
    ...
    private <V> V logTime(String actionName, Callable<V> statement) throws Exception {
        ...
        try {
            ...
            long delta = start.until(finish, ChronoUnit.MILLIS);
            log.debug("Execution of {} took: {}", actionName, delta);
        }
    }
